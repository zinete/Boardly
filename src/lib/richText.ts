...
export function renderMarkdownToHtml(markdown: string): string {
  ...
  // 4. Parse Italic (*text*)
  html = html.replace(/\*(.*?)\*/g, '<em class="italic text-zinc-700 dark:text-zinc-400">$1</em>');

  // 5. Parse Checkboxes (- [ ] and - [x])
  html = html.replace(/^(\s*)-\s+\[ \]\s+(.*?)$/gm, '<div class="flex items-center gap-2 my-1 text-zinc-600 dark:text-zinc-400"><input type="checkbox" disabled class="rounded border-zinc-300 dark:border-zinc-700 text-zinc-900 focus:ring-0 w-3.5 h-3.5 pointer-events-none" /> <span>$1</span></div>');
  ...
  html = html.replace(/^(\s*)-\s+(?!\[ \]|\[x\])(.*?)$/gm, '<li class="list-disc list-inside ml-2 my-1 text-zinc-600 dark:text-zinc-400">$2</li>');
